...
| `git.log`             | `{ workspaceId, path?, limit?, beforeSha? }` → `[{ sha, author, date, subject }]`. Newest-first; `limit` defaults to 50; `path` narrows to a single file's history; `beforeSha` paginates by anchoring at the parent of the given commit. |
