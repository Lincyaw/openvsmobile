...
void main() {
  testWidgets('hunks render with +/- markers and per-line content', (
    tester,
  ) async {
    ...
    expect(
      find.byKey(const ValueKey<String>('diff-line:old:1')),
      findsOneWidget,
    );
    expect(
      find.byKey(const ValueKey<String>('diff-line:new:1')),
      findsOneWidget,
    );
    expect(
      find.byKey(const ValueKey<String>('diff-line:old:2')),
      findsOneWidget,
    );
    expect(
      find.byKey(const ValueKey<String>('diff-line:new:2')),
      findsOneWidget,
    );
    expect(
      find.byKey(const ValueKey<String>('diff-line:new:3')),
      findsOneWidget,
    );
  ...
  testWidgets('isBinary: true renders the binary placeholder, not hunks', (
    tester,
  ) async {
    final appState = AppState(client: BackendClient());
    addTearDown(appState.dispose);
  ...
  testWidgets('tooLarge: true renders the oversize placeholder', (
    tester,
  ) async {
    final appState = AppState(client: BackendClient());
    addTearDown(appState.dispose);
  ...
  testWidgets('plugin command receives structured diff selection context', (
    tester,
  ) async {
    final appState = AppState(client: BackendClient());
    addTearDown(appState.dispose);
    ...
    final selectable = tester.widget<SelectableText>(
      find.widgetWithText(SelectableText, 'added line A'),
    );
    ...
    expect(selection['source'], 'diff');
    expect(selection['workspaceId'], 'ws-1');
    expect(selection['path'], 'lib/foo.dart');
    expect(selection['relativePath'], 'lib/foo.dart');
    expect(selection['text'], 'added');
    expect(selection['diffLineKind'], 'add');
    expect(selection['oldRange'], isNull);
    final newRange = selection['newRange'] as Map<String, dynamic>;
    expect(newRange['start'], {'line': 2, 'column': 1});
    expect(newRange['end'], {'line': 2, 'column': 6});
    expect(newRange['endExclusive'], isTrue);
  });
}
